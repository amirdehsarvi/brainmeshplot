...
#' @param ridge.count number of vertical ridge lines
...
waddingtonPlot <- function(branches = c(1,2,4),
  ...
  skeleton.curves <- lapply(1:length(branches), function(x){
    bias <- horizontal_distortion[[x]]
    sections <- unname(stats::quantile(1:bottom_width, probs = bias/sum(bias), type = 1))
  ...
  for(i in 1:length(vertical_distortion)){
  ...
  # plot
  p <- ggplot() +
    ggridges::geom_ridgeline(data = ggData,
